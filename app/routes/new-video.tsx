...
const videoFormSchema = z.object({
  originalUrl: z.string().min(1, "YouTube URL is required").regex(youtubeRegex, "Invalid YouTube URL"),
...
export default function NewVideo() {
    ...
    const onSubmit = async (data: any) => {
      ...
      try {
        const response = await ky.post(`${apiUrl}/videos`, {json: payload, throwHttpErrors: false});
        ...
        if (response.ok) {
          ...
          alert(`Failed to submit video. Server returned: ${response.status} ${response.statusText}`);
        }
      } catch (error) {
          ...
          <div className="grid grid-cols-1 flex-2/3">
            <Form method="post" onSubmit={handleSubmit(onSubmit)}>
              <div className="flex flex-col">
                ...
                />
                {errors.originalUrl && <p className="text-red-500 m-2">{errors.originalUrl.message}</p>}
              </div>
              <div className="flex flex-col">
                ...
                <input 
                  ...
                  className="bg-white rounded-md m-2 py-1 px-3 placeholder:text-gray-500 text-slate-800"
                />
                {errors.title && <p className="text-red-500 m-2">{errors.title.message}</p>}
              </div>
              <div className="flex flex-col">
                ...
                <input 
                  ...
                  className="bg-white rounded-md m-2 py-1 px-3 placeholder:text-gray-500 text-slate-800"
                />
                {errors.uploadedAt && <p className="text-red-500 m-2">{errors.uploadedAt.message}</p>}
              </div>
              <div className="flex flex-col">
              ...
              <div className="flex flex-col">
                ...
                <textarea
                  ...
                  className="bg-white rounded-md m-2 py-1 px-3 placeholder:text-gray-500 text-slate-800"
                />
                {errors.review && <p className="text-red-500 m-2">{errors.review.message}</p>}
              </div>
              <div className="flex flex-row justify-between">
              ...
              <div className="flex gap-3">
                <button 
                  ...
                  className={`bg-sky-400 rounded-full px-3 py-2 m-2 text-black font-medium ${isSubmitting ? 'opacity-50 cursor-not-allowed' : 'hover:bg-sky-500 transition-colors'}`}
                >
